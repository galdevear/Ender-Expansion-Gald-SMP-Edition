...
execute as @s[scores={fe.variable=1..}] at @s if block ~ ~2 ~ minecraft:bedrock if block ~ ~1 ~ minecraft:bedrock unless entity @e[type=minecraft:ender_dragon,tag=fe.mob,tag=fe.ender_dragon,distance=..300] run function far_end:structures/exit_portal/respawn/conditions
